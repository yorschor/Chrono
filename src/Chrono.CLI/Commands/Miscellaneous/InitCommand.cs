...
public class InitCommand : Command<InitCommand.Settings>
{
	...
	public override int Execute(CommandContext context, Settings settings, CancellationToken cancellationToken)
	{
		...
		var packagesConfigPaths =
			Directory.EnumerateFiles(projectDirectory, "packages.config", SearchOption.AllDirectories);
		...
		if (packagesConfigExists)
		{
			foreach (var path in packagesConfigPaths)
			{
				AnsiConsole.MarkupLine(
					$"[yellow]Warning: packages.config found at '{path}'. This might interfere with Chrono![/]");
			}
		}
		else
		{
		...
		if (versionFileExists || buildPropsExists)
		{
			proceedAfterPreflight =
				AnsiConsole.Confirm(
					"One or more files already exist. Do you wish to proceed (files might be overwritten)?", false);
		}
		...
		var initialVersion =
			AnsiConsole.Prompt(
				new TextPrompt<string>("What is your initial version for the project?").DefaultValue("0.0.1.0"));
		...
		var variantString = AnsiConsole.Prompt(
			new SelectionPrompt<string>()
				...
				.AddChoices("(local) Minimal", "(local) Default", "(local) Expanded", "(remote inherited) Default",
					"(remote inherited) Expanded"));
		...
		var includeBuildProps =
			AnsiConsole.Confirm("Do you need a Directory.Build.props file to reference Chrono in multiple projects?",
				!buildPropsExists);
		...
		var writeFiles =
			AnsiConsole.Confirm(
				"Proceed with writing these files to disk? ([red]Warning:[/] [bold]existing[/] files may be [bold]overwritten![/])",
				false);
		if (writeFiles)
		{
