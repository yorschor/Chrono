...
| {commitHash} | Full hash of the commit
| {commitShortHash} | Short hash of the commit
|===

TIP: If `{commitShortHash}` sits behind a `.` in your schema, prefix it with a letter (e.g.
`g{commitShortHash}`) - a hash that happens to be all digits with a leading zero is invalid SemVer
and NuGet will reject the whole version.
